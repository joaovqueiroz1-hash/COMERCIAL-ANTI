...
function LeadCard({ lead, profiles, onClick }: { lead: Lead; profiles: any[]; onClick?: () => void }) {
  const now = new Date();
  ...
  return (
    <div onClick={onClick} className="card-premium p-3 cursor-pointer hover:border-primary/40 transition-all animate-fade-in">
      <div className="flex items-start justify-between mb-1">
        ...
        {hot && <Flame size={14} className="text-warning shrink-0 ml-1" />}
      </div>
...
function SortableLeadCard({ lead, profiles, onClick }: { lead: Lead; profiles: any[]; onClick?: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <LeadCard lead={lead} profiles={profiles} onClick={onClick} />
    </div>
  );
}
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={(e) => setActiveId(e.active.id as string)} onDragEnd={handleDragEnd}>
        <div className="flex gap-3 overflow-x-auto pb-4" style={{ minHeight: 'calc(100vh - 200px)' }}>
          {PIPELINE_COLUMNS.map((col) => {
            ...
            const borderTop = col.key === 'fechado' ? 'border-t-2 border-t-success' : col.key === 'perdido' ? 'border-t-2 border-t-destructive' : '';
            return (
              <DroppableColumn key={col.key} id={col.key} className={`min-w-[220px] w-[220px] shrink-0 card-premium flex flex-col ${borderTop}`}>
                <div className="p-3 border-b border-border flex items-center justify-between">
                  ...
                  <button className="text-muted-foreground hover:text-primary transition-colors"><Plus size={14} /></button>
                </div>
                <SortableContext id={col.key} items={columnLeads.map(l => l.id)} strategy={verticalListSortingStrategy}>
                  <div className="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-280px)]">
                    {columnLeads.map(lead => (
                      <SortableLeadCard key={lead.id} lead={lead} profiles={profiles} onClick={() => handleCardClick(lead)} />
                    ))}
